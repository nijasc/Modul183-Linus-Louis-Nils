import {aq as j, ar as A, c as J, K, M as z, p as M} from "./copilot-CDIH58Wx.js";
...
class V extends z {
    ...
    repositionInPopover(t) {
        ...
        if (!o)
            return;
        const n = o.getBoundingClientRect(), h = 16, d = n.top - h, l = window.innerHeight - h - n.bottom,
            m = (t.position ?? t.getAttribute("position") ?? "bottom").startsWith("top"), p = m ? d : l, i = m ? l : d;
    ...
    requestLayoutUpdate() {
        ...
        return e ? new Promise((r) => {
            requestAnimationFrame(() => {
                ...
                const h = o._overlayElement, d = h?.shadowRoot?.querySelector('[part="overlay"]'),
                ...
                const m = Math.max(this.scrollWidth, this.offsetWidth, n.width),
                    p = Math.max(this.scrollHeight, this.offsetHeight, n.height), i = d.getBoundingClientRect(),
                    v = l.getBoundingClientRect(), W = Math.max(0, i.width - v.width),
                    N = Math.max(0, i.height - v.height), s = b?.getBoundingClientRect(),
                    P = !!s && s.width > 0 && s.height > 0,
                    L = P ? Math.max(0, i.left - s.left) + Math.max(0, s.right - i.right) : 0,
                    q = P ? Math.max(0, i.top - s.top) + Math.max(0, s.bottom - i.bottom) : 0, a = 16,
                    k = this.getPreferredMaxWidth(), G = Math.floor(window.innerHeight * 2 / 3),
                    F = Math.max(0, this.getPreferredWidth()), I = Math.max(0, this.getPreferredHeight()),
                    T = Math.min(k, Math.max(120, window.innerWidth - a * 2)),
                    $ = Math.min(G, Math.max(120, window.innerHeight - a * 2)), f = Math.max(
                        120,
                        Math.min(
                            T,
                ...
                const y = e.position, H = Number.parseFloat(o.getAttribute("top") ?? ""),
                    R = Number.parseFloat(o.getAttribute("left") ?? ""), U = y?.top ?? (Number.isNaN(H) ? 0 : H),
